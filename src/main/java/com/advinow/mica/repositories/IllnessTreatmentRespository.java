 ...
 */
@Repository
public interface IllnessTreatmentRespository extends Neo4jRepository<IllnessTreatment,Long> {
	...
	@Query("MATCH (illnessTreatment:IllnessTreatment)  WITH illnessTreatment MATCH p=(illnessTreatment)-[*0..4]-(m) RETURN p")
	Iterable<IllnessTreatment> getIllnessTreatments();
	
    @Query("MATCH (illnessTreatment:IllnessTreatment)  where toUpper(illnessTreatment.icd10Code) = $icd10Code   WITH illnessTreatment MATCH p=(illnessTreatment)-[*0..4]-(m) RETURN p")
    IllnessTreatment findTreatmentsByIcd10Code(String icd10Code);
    ...
    @Query("MATCH(illness:IllnessTreatment)-[itr]->(treatment:Treatment) where toUpper(illness.icd10Code) = $icd10Code  DETACH DELETE  illness,treatment")
    ...
    @Query("MATCH(illness:IllnessTreatment)-[itr]->(treatment:Treatment)-[tr]->(treatmentGroup:TreatmentGroup)-[drugs]-(drug) where toUpper(illness.icd10Code) =$icd10Code  DETACH DELETE  illness,treatment,treatmentGroup,drug")
    ...
    @Query("MATCH(illness:IllnessTreatment)-[itr]->(treatment:Treatment)-[tr]->(treatmentGroup:TreatmentGroup)-[drg]-(drug) where toUpper(illness.icd10Code) = $icd10Code optional match(drug)-[src]-(ts:rxNorms)    DETACH DELETE  ts")
    void deleteIllnessTmtDrugRxnorms(String icd10Code);
    
    @Query("MATCH(illness:IllnessTreatment)-[itr]->(treatment:Treatment)-[tr]->(treatmentGroup:TreatmentGroup)-[drg]-(drug) where toUpper(illness.icd10Code) = $icd10Code optional match(drug)-[src]-(ts:TreatmentPolicy)    DETACH DELETE  ts")
